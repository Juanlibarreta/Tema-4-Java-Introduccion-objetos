...
public class Persona implements Cloneable, Comparable<Persona>, AutoCloseable, Comparator<Persona>{
    private int edad = 0;
    ...
    private  VetoableChangeSupport cambiar = new VetoableChangeSupport(this);
    ...
    public Persona setEdad(int v){
        if(v < 0 || v >120){
            throw new IllegalArgumentException("Edad entre 0 y 120");
        }
    ...
    public Persona setAltura(double v){
        if(v < 0.3 || v > 2.2){
            throw new IllegalArgumentException("Altura entre 0.3 y 2.2");
        }
        if(new BigDecimal(String.valueOf(v)).scale() > 2){
            throw new IllegalArgumentException("Altura tiene maximo 2 decimales");
        }
    ...
    public Persona setPeso(double v){
        if(v < 1.7 || v > 140){
            throw new IllegalArgumentException("Peso entre 1.7 y 140");
        }
        if(new BigDecimal(String.valueOf(v)).scale() > 1){
            throw new IllegalArgumentException("Peso debe tener 1 decimal maximo");
        }
        peso = v;
        return this;
    }
    ...
    private String setNOMBRE(String v){
        if(!v.matches("[A-ZÑÁÉÍÓÚÜ][a-zñáéíóúü]+(\\s[A-ZÑÁÉÍÓÚÜ][a-zñáéíóúü]+){0,2}")){
            throw new IllegalArgumentException("Nombre debe ser primera en mayuscula, resto en minusculas");
        }
        return v;
    }
    ...
    public String toString(){
        ...
        if(mascota == null){
            ...
            "Altura: " + getAltura() + "\n" +
            "Peso " + getPeso() + "\n" + 
            "Nombre mascota: " + getMascota().getNombre() + "\n" +
